...
def get_stock_data(symbol):
    ...
    for attempt in range(3):  # Retry up to 3 times
        try:
            ...
            # Fetch historical data with a longer timeout
            history = ticker.history(period="3mo", timeout=10)
            info = ticker.info
            
            if history.empty or not info or 'symbol' not in info:
                logger.warning(f"No data available for {symbol}.TW, attempt {attempt + 1}")
            ...
            if current_price is None or prev_close is None:
                logger.warning(f"Invalid price data for {symbol}.TW")
                time.sleep(2)
                continue
            ...
            technical = {
                'ma50': round(sma50[-1], 2) if not sma50.empty and not pd.isna(sma50[-1]) else 'N/A',
                'rsi': round(rsi[-1], 2) if not rsi.empty and not pd.isna(rsi[-1]) else 'N/A',
                'macd': round(macd_value[-1], 2) if not macd_value.empty and not pd.isna(macd_value[-1]) else 'N/A',
            ...
            metrics = {
                'pe': round(info.get('trailingPE', 'N/A'), 2) if info.get('trailingPE') and not pd.isna(info.get('trailingPE')) else 'N/A',
                'pb': round(info.get('priceToBook', 'N/A'), 2) if info.get('priceToBook') and not pd.isna(info.get('priceToBook')) else 'N/A',
                'revenue_growth': round(info.get('revenueGrowth', 'N/A'), 2) if info.get('revenueGrowth') and not pd.isna(info.get('revenueGrowth')) else 'N/A',
                'eps_growth': round(info.get('earningsGrowth', 'N/A'), 2) if info.get('earningsGrowth') and not pd.isna(info.get('earningsGrowth')) else 'N/A'
            }
